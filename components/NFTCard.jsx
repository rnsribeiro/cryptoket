    ...
    return (
        <Link href={{ pathname: '/nft-details', query: { nft } }} >
            <div className='flex-1 min-w-215 max-w-max xs:max-w-none sm:w-full sm:min-w-155 
                ...
                <div className='relative w-full h-52 sm:h-36 minmd:h-60 
                minlg:h-300 rounded-2xl overflow-hidden'>
                ...
                <div className='mt-3 flex flex-col'>
                    ...
                    <div className='flexBetween mt-1 minlg:mt-3 flex-row xs:flex-col xs:items-start xs:mt-3'>
                        <p className='font-poppins dark:text-white text-nft-black-1 font-semibold text-xs minlg:text-lg'>
                            {nft.price} <span className='normal'>{nftCurrency}</span></p>
                        <p className='font-poppins dark:text-white text-nft-black-1 font-semibold text-xs minlg:text-lg'>
                            {shortenAddress(onProfilePage ? nft.owner : nft.seller)}</p>
                    </div>
                </div>
            </div>
        </Link>
